...
for ($i = 1; $i <= $cant_comidas; $i++) {
    ...
    $result2 = mysqli_query($link, "SELECT * FROM compras WHERE usuario='$user' AND id_bebidas='$idB'");
    ...
    if($cB>0){
        if ($row) {
            ...
            mysqli_query($link,"UPDATE compras SET cantidad='$nuevaCantidad', precio='$nuevoPrecio' WHERE usuario='$user' AND id_bebidas='$idB'");
            echo "datos actualizados";
                ...
                echo"$nuevoPrecio";
                mysqli_query($link,"INSERT INTO compras(usuario,id_bebidas,cantidad,precio,imagen) VALUES ('$user','$idB','$cB','$nuevoPrecio','$ima')");
